...
for files in data:
    ...
    for i in range(1, count + 1):
        ...
        # Crop image
            cropped_image, adjusted_x_coords, adjusted_y_coords = crop_image(image, x_center, y_center, width, height, x_coords,
                                                                             y_coords)
...
"""# Load image
image = cv2.imread("./7/4.jpg")

# Annotation string
annotation = "0 0.49 0.38 0.26 0.42 0.41,0.35 0.52,0.32 0.46,0.42 0.45,0.50 0.54,0.47"

# Parse YOLO annotation
class_id, x_center, y_center, width, height, x_coords, y_coords = parse_yolo_annotation(annotation)

# Crop image
cropped_image, adjusted_x_coords, adjusted_y_coords = crop_image(image, x_center, y_center, width, height, x_coords, y_coords)

# Draw points on cropped image
cropped_image_with_points = draw_points(cropped_image.copy(), adjusted_x_coords, adjusted_y_coords)

# Display cropped image with points
cv2.imshow("Cropped Image with Points", cropped_image_with_points)
cv2.waitKey(0)
